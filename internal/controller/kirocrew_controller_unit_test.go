...
func TestUpsertConditionReplacesInPlace(t *testing.T) {
	conditions := []metav1.Condition{
		{Type: kirocrewv1alpha1.ConditionReady, Status: metav1.ConditionFalse, Reason: "PodNotReady"},
	}
	upsertCondition(&conditions, metav1.Condition{
		Type: kirocrewv1alpha1.ConditionReady, Status: metav1.ConditionTrue, Reason: "Serving",
	})

	if len(conditions) != 1 {
		t.Fatalf("got %d conditions, want the existing one replaced", len(conditions))
	}
	if conditions[0].Status != metav1.ConditionTrue || conditions[0].Reason != "Serving" {
